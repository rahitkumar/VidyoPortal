...
public class X500PrincipalHelper {
	...
	public final static String attrEMAIL2 = "1.2.840.113549.1.9.1";// OID
	...
	public String getEMAILADDRESS() {
		String e = findPart(attrEMAIL);
		if (StringUtils.isEmpty(e)){
			e=findPart(attrEMAIL2);
			if (StringUtils.isNotEmpty(e)) {
				try {
					e= new String(Hex.decodeHex(e.substring(5,e.length()).toCharArray()));
				}catch(DecoderException de) {
					de.printStackTrace(); //ignore
				}
			}
		}
		return e;
	...
	private void parseDN(String dn) throws IllegalArgumentException {
		...
		while (startIndex < dn.length()) {
			...
			nameValues.add(dn.substring(startIndex, endIndex).replaceAll("\\\\,", ","));
