...
public class TenantHostnameMatchFilter extends GenericFilterBean implements
		InitializingBean, Ordered {

	protected static final Logger logger = LoggerFactory.getLogger(TenantHostnameMatchFilter.class);
		...
		try {
			...
			if (host != null && !host.equalsIgnoreCase("localhost")) {
				try {
					...
					logger.error("Tenant by URL returned error {}", e.getMessage());
				}
			}
			...
			// if multi-tenant and can't determine tenant, send HTTP 403
			boolean multiTenant = this.tenantService.isMultiTenant();
