...
HA_STATES CNegotiationNoLink::Handler( HA_EVENTS p_event )
{
   ...
   switch( p_event ) {
      ...
      case NPNG:
         ...
         if ( m_max_negotiate-- <= 0 ){
            printf("Max negotiation has been reached!\n"); 
            next_state =  ACTIVE_NOSTANDBY;
         }else
            printf("m_max_negotiate: %d\n", m_max_negotiate);
      ...
      case HB_REQ:
           if ( ( cp.Preferred() && cCI->PeerPriority() != 'P' ) || cCI->MyRole() == ACTIVE_ROLE ) {
              ...
              cmu->LogMsg( INFO, "NegotiationStateNoLink(): NAK'ing Peer. My Role: %s", cCI->StrRole(cCI->MyRole()));
